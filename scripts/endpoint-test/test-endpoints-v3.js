...
async function test1_chat_v1_simple() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 1: /api/ide/v1/chat - simple');
...
async function test2_chat_v1_with_function() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 2: /api/ide/v1/chat - with function and config');
  console.log('='.repeat(60));
...
async function test3_llm_raw_chat() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 3: /api/ide/v1/llm_raw_chat');
  console.log('='.repeat(60));
...
async function test4_llm_raw_chat_with_config() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 4: /api/ide/v1/llm_raw_chat - with full config');
  console.log('='.repeat(60));
...
async function test5_chat_v1_with_current_config() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 5: /api/ide/v1/chat - with current_config_info');
  console.log('='.repeat(60));
...
async function test6_llm_raw_chat_prompt() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 6: /api/ide/v1/llm_raw_chat_prompt');
  console.log('='.repeat(60));
...
async function main() {
  ...
  const r1 = await test1_chat_v1_simple();
  results.push({ test: 'chat_v1_simple', status: r1?.status, isSSE: r1?.isSSE });
  await sleep(3000);

  const r2 = await test2_chat_v1_with_function();
  results.push({ test: 'chat_v1_with_function', status: r2?.status, isSSE: r2?.isSSE });
  await sleep(3000);

  const r3 = await test3_llm_raw_chat();
  results.push({ test: 'llm_raw_chat', status: r3?.status, isSSE: r3?.isSSE });
  await sleep(3000);

  const r4 = await test4_llm_raw_chat_with_config();
  results.push({ test: 'llm_raw_chat_with_config', status: r4?.status, isSSE: r4?.isSSE });
  await sleep(3000);

  const r5 = await test5_chat_v1_with_current_config();
  results.push({ test: 'chat_v1_with_current_config', status: r5?.status, isSSE: r5?.isSSE });
  await sleep(3000);

  const r6 = await test6_llm_raw_chat_prompt();
  results.push({ test: 'llm_raw_chat_prompt', status: r6?.status, isSSE: r6?.isSSE });

  console.log('\n\n' + '='.repeat(60));
  console.log('SUMMARY');
  console.log('='.repeat(60));
  for (const r of results) {
    console.log(`  ${r.test}: status=${r.status}, isSSE=${r.isSSE}`);
  }
}
