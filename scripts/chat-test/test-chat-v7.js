...
function generateId() {
  ...
  let result = '';
  for (let i = 0; i < 24; i++) result += chars[Math.floor(Math.random() * 16)];
  return result;
}

async function main() {
  const sessionId = generateId() + 'a1b2';
  const taskId = generateId() + 'c3d4e5f6';
  const messageId = generateId() + 'g7h8i9j0';

  console.log('=== Test: /api/agent/v3/create_agent_task with minimal body ===');
