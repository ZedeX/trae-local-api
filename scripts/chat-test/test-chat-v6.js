...
async function main() {
  console.log('=== Step 1: Check token validity via get_detail_param ===');
  try {
    const resp = await makeRequest('POST', '/api/ide/v1/get_detail_param', {
      function: 'chat_v3',
      need_prompt: false,
      poly_prompt: false,
    });
    ...
    if (resp.status === 200) {
      ...
      console.log('Token is valid! Config count:', json.config_info_list ? json.config_info_list.length : 0);
    } else {
      console.log('Token may be invalid. Response:', resp.body.substring(0, 500));
      return;
    }
  } catch (e) {
  ...
  try {
    ...
    if (resp.isSSE) {
      console.log('SSE Response (first 5000 chars):', resp.body.substring(0, 5000));
    } else {
  ...
  console.log('\n=== Step 3: Test /api/ide/v1/llm_raw_chat ===');
  const rawBody = {
    messages: [
      { role: 'user', content: 'Say "Hello" and nothing else.' }
    ],
  ...
  try {
    const resp = await makeRequest('POST', '/api/ide/v1/llm_raw_chat', rawBody);
    console.log('Status:', resp.status);
    console.log('Content-Type:', resp.headers['content-type']);
    if (resp.isSSE) {
      console.log('SSE Response (first 5000 chars):', resp.body.substring(0, 5000));
