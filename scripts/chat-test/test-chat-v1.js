...
async function testChatV1Minimal() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 1: /api/ide/v1/chat - minimal body');
  ...
  try {
    ...
    if (resp.isSSE) {
      console.log('SSE Response (first 1000 chars):', resp.body.substring(0, 1000));
    } else {
...
async function testChatV1WithModel() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 2: /api/ide/v1/chat - with model');
  console.log('='.repeat(60));
...
async function testChatV1WithConfig() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 3: /api/ide/v1/chat - with config_name and function');
  console.log('='.repeat(60));
...
async function testChatV1FullConfig() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 4: /api/ide/v1/chat - with full config_info');
  console.log('='.repeat(60));
  ...
  const body = {
    ...
    current_config_info: {
      ...
      model_detail_list: [
        {
          model_name: 'doubao-1-6-pro-32k',
          provider_model_name: 'doubao-1-6-pro-32k',
          provider: 'doubao',
        }
      ]
    },
...
async function testLlmRawChatV2Full() {
  console.log('\n' + '='.repeat(60));
  console.log('Test 5: /api/ide/v2/llm_raw_chat - with full params');
  console.log('='.repeat(60));
  ...
  try {
    const resp = await makeRequest('POST', '/api/ide/v2/llm_raw_chat', body);
    console.log('Status:', resp.status);
    if (resp.isSSE) {
      console.log('SSE Response (first 1000 chars):', resp.body.substring(0, 1000));
    } else {
...
async function main() {
  await testChatV1Minimal();
  console.log('\nWaiting 3 seconds...');
  await new Promise(r => setTimeout(r, 3000));
  
  await testChatV1WithModel();
  console.log('\nWaiting 3 seconds...');
  await new Promise(r => setTimeout(r, 3000));
  
  await testChatV1WithConfig();
  console.log('\nWaiting 3 seconds...');
  await new Promise(r => setTimeout(r, 3000));
  
  await testChatV1FullConfig();
  console.log('\nWaiting 3 seconds...');
  await new Promise(r => setTimeout(r, 3000));
  
  await testLlmRawChatV2Full();
}
