...
async function main() {
  console.log('=== Test 1: GET /api/ide/v1/model_list ===');
  try {
    const resp = await makeRequest('GET', '/api/ide/v1/model_list', null);
    console.log('Status:', resp.status);
    try {
      const json = JSON.parse(resp.body);
      console.log('Response (formatted):', JSON.stringify(json, null, 2).substring(0, 5000));
    } catch (e) {
  ...
  console.log('\n=== Test 2: GET /api/ide/v1/get_model_list ===');
  try {
    const resp = await makeRequest('GET', '/api/ide/v1/get_model_list', null);
    console.log('Status:', resp.status);
    try {
      const json = JSON.parse(resp.body);
      console.log('Response (formatted):', JSON.stringify(json, null, 2).substring(0, 5000));
  ...
  console.log('\n=== Test 3: GET /api/ide/v1/get_detail_param ===');
  try {
    const resp = await makeRequest('GET', '/api/ide/v1/get_detail_param', null);
    console.log('Status:', resp.status);
    try {
      const json = JSON.parse(resp.body);
      console.log('Response (formatted):', JSON.stringify(json, null, 2).substring(0, 5000));
  ...
  console.log('\n=== Test 4: POST /api/ide/v1/get_detail_param ===');
  try {
    const resp = await makeRequest('POST', '/api/ide/v1/get_detail_param', { config_name: 'Doubao_1_6' });
    console.log('Status:', resp.status);
    try {
      const json = JSON.parse(resp.body);
      console.log('Response (formatted):', JSON.stringify(json, null, 2).substring(0, 5000));
