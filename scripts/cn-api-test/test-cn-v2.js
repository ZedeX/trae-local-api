...
async function testCreateAgentTask(modeType, agentType) {
  console.log(`\n=== Test create_agent_task (mode_type=${modeType}, agent_type=${agentType}) ===`);
  ...
  if (contentType.includes('stream') || contentType.includes('event-stream')) {
    ...
    console.log('Non-stream response:', text.substring(0, 1000));
    return true;
  }
}
...
async function testGetDetailParam() {
  ...
  if (resp.status === 200) {
    try {
      ...
      fs.writeFileSync('d:/_program/Trae/zx-test/detail-param-response.json', JSON.stringify(data, null, 2), 'utf-8');
      console.log('Saved to detail-param-response.json');
      if (data.data) {
...
async function main() {
  try {
    ...
    const modeTypes = [0, 1, 'Auto', 'Manual'];
    const agentTypes = ['builder_v3', 'builder'];

    for (const mt of modeTypes) {
      for (const at of agentTypes) {
        const success = await testCreateAgentTask(mt, at);
        if (success) {
          console.log('\n*** SUCCESS! ***');
          return;
        }
