const { getAuthInfo, getDeviceIds, buildStreamHeaders, buildCommonHeaders, refreshTokenIfNeeded, getDeviceInfo, getIdeVersion, getIdeVersionCode } = require('./src/auth');
...
async function test() {
  try {
    ...
    console.log('=== Test CN API host with SG token ===');

    const cnHost = 'https://trae-api-cn.mchost.guru';
    const endpoint = `${cnHost}/api/agent/v3/create_agent_task`;
