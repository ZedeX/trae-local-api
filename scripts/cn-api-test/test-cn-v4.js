...
async function testWithModelConfig() {
  console.log('=== Step 1: Get model config ===');
  const modelConfig = await getModelConfig('builder_v3', 'Doubao_1_6');
  if (!modelConfig) {
    console.log('Model config not found for Doubao_1_6');
    const allConfigs = await getModelConfig('builder_v3', null);
    console.log('Trying to list all configs...');
    return;
  }
  
  console.log('Config found:', modelConfig.config_name);
  console.log('Model details count:', modelConfig.model_detail_list?.length);
  ...
  console.log('Model name:', modelDetail.model_name);
  console.log('Has encrypted params:', !!modelDetail.encrypted_model_params);
  ...
  console.log('\n=== Step 2: Create agent task with model config ===');
...
testWithModelConfig().catch(console.error);
